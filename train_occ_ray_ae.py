...
def main():
    ...
    train_dataset = OccRayDataset(
        range = 1,
        resolution = config.OCC_RAY_RESOLUTION,
        len = 1024,#*16
        n_occ_fcn_samples = config.N_OCC_FCN_SAMPLES,
    )
    ...
    occ_ray_encoder = OccRayEncoder(
        ...
        ksize_list = [45],
        stride_list = [1],
        fc_channel_list = [1024, 1024, 1024, 1024, 1024, config.OCC_RAY_LATENT_DIM],
        # cnn_channel_list = [2, 16, 32, 64, 128, 256, 512, 1024],
        # ksize_list = [3, 3, 3, 3, 3, 3, 3],
        # stride_list = [1, 2, 1, 2, 1, 2, 1],
        # fc_channel_list = [1024, 1024, 1024, config.OCC_RAY_LATENT_DIM],
    ).cuda()
    occ_ray_decoder = OccRayDecoder(
        fc_channel_list = [config.OCC_RAY_LATENT_DIM+1, 1024, 1024, 1024, 1024, 1],
    ).cuda()
    ...
    for epoch in range(config.N_EPOCHS):
        for batch_idx, batch in tqdm(enumerate(train_dataloader), 'Epoch #{}/{}'.format(epoch+1, config.N_EPOCHS)):
            is_last_batch = not (batch_idx+1) < len(train_dataloader)
            z = occ_ray_encoder(batch['occ_ray_rasterized'].reshape((config.BS, 1, config.OCC_RAY_RESOLUTION)).cuda(), batch['grid'].reshape((config.BS, 1, config.OCC_RAY_RESOLUTION)).cuda())
            occ_fcn_vals_pred = occ_ray_decoder(
                z.reshape((config.BS, 1, config.OCC_RAY_LATENT_DIM)).expand((-1, config.N_OCC_FCN_SAMPLES, -1)).reshape((config.BS*config.N_OCC_FCN_SAMPLES, config.OCC_RAY_LATENT_DIM)),
                batch['radial_samples'].reshape((config.BS*config.N_OCC_FCN_SAMPLES, 1)).cuda(),
            ).reshape((config.BS, config.N_OCC_FCN_SAMPLES))
            ...
            optimizer.step()
            optimizer.zero_grad()
            if is_last_batch or (config.N_BATCHES_LOG_INTERVAL is not None and global_batch_cnt % config.N_BATCHES_LOG_INTERVAL == 0):
